...
const Word2vec = (props) => {
    ...
    const handleSearch = () => {
        if (!input)
            return;
        Axios.post(API.cluster, {model:"w2v", number:input})
            .then(res => {
                props.setW2vData(res.data);
            })
    }

    let result;
    props.w2vData.length ? result = props.w2vData : result = props.data;
    result = result
        .filter(element => element.model_name === "w2v")
        .sort((a, b) => b.items_i.length - a.items_i.length);
    return (
        <>
            <div style={{margin:"0 auto", textAlign:"center"}}>
                <img src={require("../Assets/Imgs/answer3.png")} style={{width:'120px'}}/>
                <span className="title">word2vec结果</span>
            </div>
                        ...
                        <Collapse defaultActiveKey={['0']}  className="site-collapse-custom-collapse">
                            {
                                result.map((item, index) =>
                                    <Panel key={index} header={"类别 " + (index+1)} extra = {"共" +(item.items_i.length)+ "项"} className="site-collapse-custom-panel">
